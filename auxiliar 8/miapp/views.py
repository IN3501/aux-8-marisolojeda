...
def modificar_user(request):
	...
	password_antigua = request.POST['password_antigua']
	password_nueva = request.POST['password_nueva']

	user = request.user
	user = authenticate(username=usuario, password=password_antigua)

	user.set_password(password_nueva)
	user.save()
	return render(request, 'miapp/added_user.html')
...
def cambio_usuario(request):
	return render(request, 'miapp/cambio_usuario.html')
